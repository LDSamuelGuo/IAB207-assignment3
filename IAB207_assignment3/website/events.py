...
def show(id):
    ...
    organiser_events = Event.query.filter_by(headliner=event.headliner).all()
    return render_template('events/event_details.html', event=event, form=comments_form, booking_form=booking_form, organiser_events=organiser_events)
...
@eventbp.route('/view_all/organiser/<headliner>')
def view_events_organiser(headliner):
    organiser_events = Event.query.filter_by(headliner=headliner).filter(
        Event.event_status != 'INACTIVE').all()
    if len(organiser_events) == 1:
        return redirect(url_for('events.show', id=organiser_events[0].id))
    return render_template('events/view_events.html', heading=headliner, events=organiser_events)
